...
async function getCategoriesAndApplications() {
  ...
  const [{ data: categories, error: categoriesError }, { data: applications, error: applicationsError }] =
    await Promise.all([
...
function renderDynamicAnswers(application: WhitelistApplication) {
  if (application.answers && application.answers.length > 0) {
    return application.answers.map((item, index) => (
      <div key={`${application.id}-answer-${index}`} className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
        <h3 className="font-semibold">{item.question}</h3>
        <p className="mt-2 whitespace-pre-wrap text-white/70">{item.answer}</p>
      </div>
    ));
  }
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-4 text-white/70">
              Ahora la revisión está separada por categorías y muestra todas las respuestas del usuario.
            </p>
          </div>
              ...
              <section key={category.id} className="space-y-6">
                <div className="panel p-6">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                      <p className="text-xs uppercase tracking-[0.35em] text-white/45">Categoría</p>
                      <h2 className="mt-2 text-3xl font-black uppercase">{category.title}</h2>
                    ...
                    <div className="text-right">
                      ...
                      >
                        {category.is_active ? "Abierta" : "Cerrada temporalmente"}
                      </span>
                      <p className="mt-3 text-sm text-white/55">
                        Solicitudes: {category.applications.length}
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div className="relative z-10">
                          <div className="flex flex-wrap items-start justify-between gap-4">
                            <div>
                              ...
                              <p className="mt-2 text-white/65">
                                Categoría: {application.category_name || category.title}
                              </p>
                              ...
                              <p className="text-white/65">
                                Edad real: {application.real_age} · Edad personaje: {application.character_age}
                              </p>
                          ...
                          <div className="mt-6 grid gap-5 md:grid-cols-2">
                            {renderDynamicAnswers(application)}
                          </div>
